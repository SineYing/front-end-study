...
- Promise.all()可以将多个promise合并为一个promise统一管理，他需要接收的是一个数组，而这个数组中的每一个元素都是一个promise对象,Promise.all()返回一个全新的promise对象，内部所有的Promise完成过后这个全新的Promise才会完成，那这个promise拿到的结果会是一个数组，里面包含着每一个异步任务执行的结果
